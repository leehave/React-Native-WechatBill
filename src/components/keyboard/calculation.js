...
export default class Calculation {
  ...
  static getMoneyString = (string, index) => {
    ...
    // 点
    if (Calculation.isPoint(index) && Calculation.isAllowPoint(string)) {
      return Calculation.enterPoint(string, index)
    }
    // 删除
    if (Calculation.isRemove(index) && Calculation.isAllowRemove(string)) {
      return Calculation.enterRemove(string, index)
    }
    // 加减
    if (Calculation.isAddLess(index) && Calculation.isAllowAddLess(string)) {
      return Calculation.enterAddLess(string, index)
    }
    // 时间
    if (Calculation.isDate(index) && Calculation.isAllowDate(string)) {

    }
    // 完成
    if (Calculation.isComplete(index) && Calculation.isAllowComplete(string)) {
      return Calculation.enterComplete(string, index)
    }
    return string
  }
  ...
  static getButtonString = (index, string, date) => {
    if (index == 0) {
      ...
      return Calculation.updateComplete(string)
    }
  }
}
